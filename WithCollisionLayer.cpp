...
/*    // add "HelloWorld" splash screen"
	...
	road = Sprite::create("road1800.jpg"/*, Rect(0, 0, 300, 300)*/);
	road->setPosition(Vec2(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y));
	this->addChild(road);

	road_upper_part = Sprite::create("road1800.jpg");
...
{
	if (current->getPositionY() < -current->getBoundingBox().size.height / 2)
...
void WithCollision::checkCarsNearWithUpdate(PhysicsCar *current)
{
	...
	if (((user_car->getPositionX() - current->getPositionX()) < 2 * ROAD_LINE_WIDTH) && ((user_car->getPositionX() - current->getPositionX()) > 0 ) &&(abs(user_car->getPositionY() - current->getPositionY()) < 400))
		opt.setIsRight(true);
	if (((user_car->getPositionX() - current->getPositionX()) > -2 * ROAD_LINE_WIDTH) && ((user_car->getPositionX() - current->getPositionX()) < 0 ) &&(abs(user_car->getPositionY() - current->getPositionY()) < 400))
		opt.setIsLeft(true);
	current->setMoveOption(opt.getIsFront(), opt.getIsLeft(), opt.getIsRight());
...
{
	if ((abs(user_car->getPositionX() - currentCar->getPositionX()) < 2 * ROAD_LINE_WIDTH )&&(abs(user_car->getPositionY() - currentCar->getPositionY()) < 400))
		return false;
	else
		return true;
}
